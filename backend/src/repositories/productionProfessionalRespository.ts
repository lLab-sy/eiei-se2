...
import {
  IProductionProfessional,
  ProductionProfessional,
  Rating,
  searchProductionProfessionalResponse,
  searchReqModel,
} from "../models/userModel";
import { PipelineStage } from "mongoose";

class ProductionProfessionalRespository {
  public async updateProductionProfessional(
    newData: Partial<IProductionProfessional>,
    id: string
  ) {
    try {
      if (!id) {
  ...
  async searchProductionProfessional(
    searchReqModel: searchReqModel
  ): Promise<searchProductionProfessionalResponse> {
    try {
        ...
        matchStage.push({
          $match: {
            $or: [
              ...
              {
                middleName: {
                  $regex: searchReqModel.searchText,
                  $options: "i",
                },
              },
              {
      ...
      if (
        searchReqModel.minExperience !== undefined ||
        searchReqModel.maxExperience !== undefined
      ) {
      ...
      if (searchReqModel.minRating !== undefined) {
        matchStage.push(
          ...
          {
            $replaceRoot: {
              newRoot: { $mergeObjects: ["$doc", { avgRating: "$avgRating" }] },
            },
          },
      ...
      const totalItemsResult = await ProductionProfessional.aggregate(
        totalItemstStage
      );
      const totalItems =
      ...
      const results: IProductionProfessional[] =
        await ProductionProfessional.aggregate(matchStage);
  ...
  public async addProductionProfessionalReview(
    id: string,
    newRatingModel: Rating
  ) {
    try {
      if (!id) {
        throw new Error("Id is required");
      ...
      if (!result) {
        throw new Error(
          "Review from post already exists or Production professional not found"
        );
      }
