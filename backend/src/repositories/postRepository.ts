...
class PostRepository {
    ...
    try {
      ...
      matchStage.push({
        $match: {
          $and: [
            {
              "participants.participantID": new ObjectId(id),
            },
      ...
      throw new Error(
        "Error Get Posts history Production professional from repository: " +
          error
      );
    }
  }
  ...
  public async getPost(id: string) {
    try {
      ...
      const post: IPost | null = await Post.findById(objectId).populate([
        "postProjectRoles",
      ]);
  ...
  public async deletePost(postData: PostDTO, id: string) {
    try {
      ...
      const objectId = new ObjectId(id);
      console.log(objectId);
      const post = await Post.findOneAndDelete({ _id: objectId });
  ...
  public async createOffer(
    offerData: ParticipantDetailDTO,
    postID: string,
    productionProfessionalID: string
  ) {
    console.log("OKAY CREATE  participant");
    try {
      const productionProfessionalObjectID = new ObjectId(
  ...
  public async checkProductionProInPost(
    postID: string,
    productionProfessionalID: string
  ) {
    try {
      ...
      throw new Error(
        "Error find production professional in repository: " + error
      );
    }
  }

  public async searchPost(
    postSearchReq: PostSearchRequestModel
  ): Promise<PostSearchResponse> {
    try {
      const { searchText, postMediaTypes, roleRequirements, limit, page } =
        postSearchReq;
      ...
      const totalItemstStage: PipelineStage[] = [
        ...matchStage,
        { $count: "totalCount" },
      ];
  ...
  public async addPostReview(
    ...
    try {
      ...
      if (!result) {
        ...
        throw new Error(
          "Production professional (participantID) not found in this Post"
        );
      }
  ...
  public async getProducerOffer(
    getOfferReq: GetOfferRequestModel,
    producerID: string
    ...
    try {
      ...
      if (userId) {
        matchStage.push({
          //paticipant in post if it has
          $match: {
            "participants.participantID": new ObjectId(userId),
          },
        });
      }

      matchStage.push({
        // change roleID -> roleName
        $lookup: {
          from: "postRoleTypes",
      ...
      matchStage.push({
        $project: {
          _id: 1,
          postName: 1,
          participantsID: "$participants.participantID",
          roleName: {
  ...
  public async getPostsByProf(
    getPostByProfReq: GetPostByProfRequestModel
  ): Promise<GetPostByProfResponse> {
    try {
