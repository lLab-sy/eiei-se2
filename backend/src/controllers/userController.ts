...
class UserController {
  ...
  async updateUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (profileImageFile) {
        ...
        const updatedUser =
          role === "producer"
            ? await userService.updateProducer(userDataWithImageKey, id)
            : await userService.updateProductionProfessional(
                userDataWithImageKey,
                id
              );
        if (!updatedUser) {
        ...
        const updatedUser =
          role === "producer"
            ? await userService.updateProducer(userData, id)
            : await userService.updateProductionProfessional(userData, id);
        if (!updatedUser) {
          sendResponse(res, "error", "Failed to update User updated User");
          return;
        }
  ...
  async uploadProfileImage(req: Request, res: Response): Promise<void> {
    try {
      ...
      const returnData =
        await cloudService.uploadImageToGetURLWithDeleteCondition(
          buffer!,
          mimetype!,
          imageKey,
          id
  ...
  async search(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await userService.searchProductionProfessional(
        searchParams
      );
  ...
  async getUserReceivedReviewsByID(req: Request, res: Response): Promise<void> {
    try {
      ...
      const userReceivedReviews = await userService.getUserReceivedReviewsByID(
        userID
      );
      sendResponse(
        res,
        "success",
        userReceivedReviews,
        "Successfully get user received reviews"
      );
    } catch (err) {
      console.log(err);
      sendResponse(
        res,
        "error",
        "control layer Failed to get user received reviews"
      );
    }
  }
  ...
  async addProductionProfessionalReview(
    ...
    try {
      ...
      const user = await userService.addProductionProfessionalReview(
        id,
        newReview
      );
