...
import {
  ...
  RatingDTO,
} from "../dtos/productionProfessionalDTO";
...
class UserService {
  ...
  async updateProducer(producerDTO: ProducerDto, id: string) {
    try {
      ...
      const res = await ProducerRespository.updateProducer(
        producerDTO as IProducer,
        id
      );
      return res;
    } catch (error) {
  ...
  async updateProductionProfessional(
    productionDTO: ProductionProfessionalDtO,
    id: string
  ) {
    try {
      ...
      const res =
        await ProductionProfessionalRespository.updateProductionProfessional(
          productionDTO as IProductionProfessional,
          id
        );
      return res;
    } catch (error) {
  ...
  async searchProductionProfessional(
    searchReq: searchReqDTO
  ): Promise<PaginatedResponseDTO<ProductionProfessionalDtO>> {
    try {
      const searchModel: searchReqModel = searchReq;
      const results =
        await ProductionProfessionalRespository.searchProductionProfessional(
          searchModel
        );
      ...
      throw new Error(
        "Error in service layer when search Production Professional: " +
              ...
              r.reviews.map(async (review: reviewDTO) => {
                let producerProfileImage = review.producerProfileImage
                  ? await cloudService.getSignedUrlImageCloud(
                      review.producerProfileImage
                    )
                  : "";
                console.log(
                  "reviewProducerProfileImage",
                  review.producerProfileImage
                );
  ...
  async addProductionProfessionalReview(
    id: string,
    newReviewDTO: RatingDTO
  ): Promise<ProductionProfessionalDtO> {
    try {
      // check post status

      if (newReviewDTO.postID == undefined) {
        throw new Error(
          "Error in service layer when add Review to Production Professional : postID is 'undefined'"
        );
      }

      const post: IPost = await postRepository.getPost(
        newReviewDTO.postID.toString()
      );
      if (post.postStatus != "success") {
        throw new Error(
          "Error in service layer when add Review to Production Professional : postStatus not 'success'"
        );
      }
      ...
      const results =
        await ProductionProfessionalRespository.addProductionProfessionalReview(
          id,
          ratingModel
        );
      return results;
      ...
      throw new Error(
        "Error in service layer when add Review to Production Professional: " +
          (error as Error).message
      );
    }
  }
  ...
  async getUserReceivedReviewsByID(id: string) {
    try {
      const userReceivedReviews =
        await userRepository.getUserReceivedReviewsByID(id);
          ...
          return new ReceivedReviewsDTO({
            receivedReviews: await Promise.all(
              r.reviews.map(async (review: ReceivedReviewDTO) => {
                let reviewerProfileImageTmp =
                  typeof review.reviewerProfileImage === "string"
                    ? await cloudService.getSignedUrlImageCloud(
                        review.reviewerProfileImage
                      )
                    : review.reviewerProfileImage
                    ? await cloudService.getSignedUrlImageCloud(
                        review.reviewerProfileImage[0]
                      )
                    : "";
      ...
      console.error(
        "Error service layer fetching user received reviews",
        error
      );
      throw error;
    }
