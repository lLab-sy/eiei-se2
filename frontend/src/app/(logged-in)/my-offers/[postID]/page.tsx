// frontend/src/app/(logged-in)/my-offers/[postID]/page.tsx

"use client";
import OfferHistory, { offerHistoryInterface } from "@/components/OfferHistory";
...
import { PostData, OfferData } from "../../../../../interface";
import OfferHistoryContent from "@/components/OfferHistoryContent";
...
export default function OfferDetailContent() {
  ...
  const [offerArray, setOfferArray] = useState<Array<offerHistoryInterface>>(
    [],
  );
  ...
  useEffect(() => {
    const handleFetch = async (userId: string) => {
      const query = `?postId=${postID}&producerId=${userId}&limit=10&page=1`;
          ...
          >
            {offerArray.map((offer, index) => (
              <div key={index} className="mt-10 w-[90%]">
                <OfferHistoryContent data={offer} />
              </div>
            ))}
          </div>
        </div>
